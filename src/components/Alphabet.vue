  ...
  <div class="area" v-if="cityInfo" ref="area_scroll">
    <div class="scroll_wrap">
      ...
      <div class="hot_wrap citylist">
        ...
        <ul class="hot_city">
          <li v-for="(item,index) in cityInfo.hotCities" :key="index" @click="$emit('select-city',item)">{{item.name}}</li>
        </ul>
      </div>
      <!-- 所有城市 -->
      <div class="city_wrap">
        ...
        <div class="city_content citylist" v-for="(item,index) in keys" :key="index">
          ...
          <!-- 根据字母key展示城市名 -->
          <ul>
            <li v-for="(city,index) in cityInfo[item]" :key="index" @click="$emit('select-city',city)">{{city.name}}</li>
          </ul>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods:{
    ...
    // 点击字母实现滚动
    selectKey(index){
      const citylist = this.$refs.area_scroll.getElementsByClassName("citylist");//获取需要滚动的dom元素
      // 根据下标滚动到对应的元素上
